 ...
 citations={article.sourceUrls}/><main id="main-content" className="article-main">
    ...
    <div className="article-wrap article-layout">
      <article>
        ...
        <section className="article-sources" aria-labelledby={`${article.slug}-quellen`}><p className="eyebrow dark">Quellen</p><h2 id={`${article.slug}-quellen`}>Offizielle Informationsbasis</h2><p>Diese offiziellen Pro-Tent-Seiten dokumentieren die verwendeten Produktinformationen:</p><ol>{article.sourceUrls.map((url, index) => <li key={url}><Out href={url}>Quelle {index + 1}: {new URL(url).pathname.replace('/de-de/', '').replaceAll('/', ' / ') || 'Pro-Tent'}</Out></li>)}</ol></section>
      </article>
