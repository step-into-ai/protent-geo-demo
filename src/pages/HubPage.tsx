...
export function HubPage({ page }: { page: HubPageContent<ClaimId, HubPath> }) {
  ...
  return <Layout><Meta meta={page.meta} faqs={page.faqs} title={page.title}/><main id="main-content" className="hub-main">
    ...
    <section className="section wrap hub-articles"><div className="section-head"><p className="eyebrow dark">Vertiefendes Wissen</p><h2>Passende Wissensseiten</h2><p>Lesen Sie die Planungshilfe oder wechseln Sie direkt zur passenden Originalseite bei Pro-Tent.</p></div><div className="article-card-grid">{hubArticles.map(article => <article key={article.slug}><p className="eyebrow dark">{article.cluster}</p><h3><Link to={article.path}>{article.title}</Link></h3><p>{article.description}</p><div className="article-card-actions"><Link className="text-link" to={article.path}>Artikel lesen <span>→</span></Link><Out href={article.sourceUrls[0]}>Originalseite bei Pro-Tent</Out></div></article>)}</div></section>
