...
const homeTemplate = metadata(template, { title: 'Pro-Tent Deutschland | Wissen für mobile Markenräume', description: 'Produkt- und Planungswissen für mobile Markenräume in Deutschland – mit klar gekennzeichneten Quellen, Planungshinweisen und direktem Weg zu Pro-Tent.', canonical: `${site}/` }, 'website')
await writeFile('dist/index.html', injectRoot(injectSchema(homeTemplate, homeGraph), homeVisible))
...
const knowledgeVisible = `<main class="static-knowledge" data-static-content="true"><h1>Wissen für mobile Markenräume</h1><p>25 fundierte Ratgeber für Unternehmen in Deutschland.</p>${Object.entries(hubNames).map(([path, name]) => `<section><h2>${escapeText(name)}</h2><p><a href="..${path}/">Themenbereich öffnen</a></p><ul>${articles.filter(article => article.hubPath === path).map(article => `<li><a href="../${article.slug}/">${escapeText(article.title)}</a> – ${escapeText(article.description)}</li>`).join('')}</ul></section>`).join('')}${conceptDisclosure}</main>`
...
for (const [routePath, page] of Object.entries(pageContent.hubs)) {
  ...
  const visible = `<main class="static-hub" data-static-content="true"><nav aria-label="Brotkrumen"><a href="../">Wissenswelt</a> / ${escapeText(page.eyebrow)}</nav><article><p>${escapeText(page.eyebrow)}</p><h1>${escapeText(page.title)}</h1><p>${escapeText(page.intro)}</p><section><h2>Entscheidungskriterien</h2>${page.criteria.map(item => `<h3>${escapeText(item.title)}</h3><p>${escapeText(item.text)}</p>`).join('')}</section><section><h2>Passende Wissensseiten</h2><ul>${hubArticles.map(article => `<li><a href="../${article.slug}/">${escapeText(article.title)}</a> – ${escapeText(article.description)}</li>`).join('')}</ul></section><section><h2>Häufige Fragen</h2>${page.faqs.map(faq => `<h3>${escapeText(faq.question)}</h3><p>${escapeText(faq.answer)}</p>`).join('')}</section></article>${conceptDisclosure}</main>`
...
notFound = injectRoot(notFound, `<main><p>404</p><h1>Seite nicht gefunden</h1><p>Dieser Wissenspfad existiert nicht.</p><a href="./">Zur Startseite</a>${conceptDisclosure}</main>`)
